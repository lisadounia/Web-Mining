...
#     largest_component_size = max(len(comp) for comp in components)  # Taille de la plus grande composante
#
#     # Vérifier si les deux nœuds du pont sont toujours connectés à Social Inequality
...
#
#     # Ajouter les résultats à la liste des scores d'impact
#     bridge_impact_scores.append(((u, v), len(components), largest_component_size,
#                                   connected_to_social_inequality_u, connected_to_social_inequality_v))
#
...
# Obtenir les successeurs directs de Social Inequality
...
for node in SCC.nodes:
    if node != social_inequality_node and node not in successors:  # Exclure le nœud Social Inequality lui-même
        all_children = False
        non_children_nodes.append(node)

# Afficher les résultats
print("=== Vérification des nœuds fils de Social Inequality ===")
...
if not all_children:
    ...
    print("Tous les nœuds du SCC sont directement pointés par Social Inequality.")
